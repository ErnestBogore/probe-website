...
import { getSeoToolBySlugJa, getAllSeoToolSlugsJa } from '@/lib/seo-tools/i18n/seo-tools-config.ja';
...
export async function generateStaticParams() {
  const aiTools = getAllToolsJa();
  const seoToolSlugs = getAllSeoToolSlugsJa();
  return [
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seoTool = getSeoToolBySlugJa(slug);
  ...
  const hreflangAlternates = seoTool
    ...
    : generateToolHreflangAlternates(getEnglishSlugJa(activeTool.slug) ?? activeTool.slug, 'tool');

  return {
    title: `${activeTool.title}`,
    description: activeTool.metaDescription,
    alternates: {
      canonical: `https://www.tryanalyze.ai/free-tools/ja/${activeTool.slug}`,
      languages: hreflangAlternates,
    },
    openGraph: {
      title: `${activeTool.title}`,
      description: activeTool.metaDescription,
      url: `https://www.tryanalyze.ai/free-tools/ja/${activeTool.slug}`,
      type: 'website',
      locale: 'ja_JP',
      images: [
        {
...
export default async function JapaneseToolPage({ params }: PageProps) {
  ...
  if (seoTool) {
    ...
    const breadcrumbSchema = generateBreadcrumbSchema([
      { name: 'ホーム', href: '/' },
      { name: '無料ツール', href: '/free-tools/ja' },
      { name: seoTool.name, href: `/free-tools/ja/${seoTool.slug}` },
    ]);
    const webAppSchema = generateWebApplicationSchema({
      name: seoTool.title,
      description: seoTool.metaDescription,
      url: `https://www.tryanalyze.ai/free-tools/ja/${seoTool.slug}`,
    ...
    return (
      <>
        ...
        <SeoToolPage tool={seoTool} locale="ja" relatedTools={computeSeoRelatedTools(slug, 'ja')} />
      </>
    );
  }
