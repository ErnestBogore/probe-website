...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!tool) {
    return {
      title: 'Outil Non Trouvé | Analyze',
    };
  }
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `https://www.tryanalyze.ai/free-tools/${getEnglishSlug(tool.slug) || tool.slug}`,
        'fr': `https://www.tryanalyze.ai/free-tools/fr/${tool.slug}`,
      },
    },
    openGraph: {
      title: `${tool.title} | Analyze`,
...
export default async function FrenchToolPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      />
      <ToolPage tool={tool} locale="fr" englishSlug={englishSlug} />
    </>
  );
}
